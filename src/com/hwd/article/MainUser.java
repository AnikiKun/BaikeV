 ...
 * 主界面， 个人中心界面
 */
public class MainUser implements OnRefreshListener<ListView>, GetNewArticleTaskHelper  {
	...
	public MainUser(MainActivity act, View root) {
		this.act = act;
		this.root = root;
		
		
		String userStr = act.getDataString(BaseActivity.USER_KEY);
	...
	private void initView() {
		...
		root.findViewById(R.id.title_bar_right_btn).setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				Intent intent = new Intent(act, AddArticleActivity.class);
	...
	private void refreshData() {
		...
		task = new GetNewArticleTask(this, 0, PAGE_SIZE, user_id);
		task.startTask();
		timeOut();
	}
	...
	private void loadLastData() {
		...
		adapter = new ArticleListAdapter(getActivity(), list, PAGE_SIZE, user_id);
	...
	public void getDataResult(String result) {
		...
		if (!result.equals("")) {
			if (!result.startsWith("FAIL")) {
				...
				adapter = new ArticleListAdapter(getActivity(), artList, PAGE_SIZE, user_id);
				listView.setAdapter(adapter);
